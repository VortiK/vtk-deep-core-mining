...
local config = require('__vtk-deep-core-mining__/stdlib/config')

local Table = require('__vtk-deep-core-mining__/stdlib/utils/table')
local Math = require('__vtk-deep-core-mining__/stdlib/utils/math')
    ...
    while true do
        ...
        if trace then
            ...
            if (trace.what == 'Lua' or trace.what == 'main') and not ignored[trace.name] then
                local cur = trace.source:gsub('.*__vtk-deep-core-mining__', '__vtk-deep-core-mining__'):gsub('.*/Factorio%-Stdlib', '__vtk-deep-core-mining__')
...
local inspect = require('__vtk-deep-core-mining__/stdlib/vendor/inspect')
rawset(_ENV, 'inspect', inspect)

-- Defines Mutates
require('__vtk-deep-core-mining__/stdlib/utils/defines/color')
require('__vtk-deep-core-mining__/stdlib/utils/defines/anticolor')
require('__vtk-deep-core-mining__/stdlib/utils/defines/lightcolor')
require('__vtk-deep-core-mining__/stdlib/utils/defines/time')
...
function STDLIB.create_stdlib_globals(files)
    ...
    for glob, path in pairs(files) do
        rawset(_ENV, glob, require('__vtk-deep-core-mining__/' .. (path:gsub('%.', '/')))) -- extra () required to emulate select(1)
    end
end
